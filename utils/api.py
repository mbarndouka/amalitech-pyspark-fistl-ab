...
- Retry on transient HTTP errors (5xx, network failures) and rate-limit
  responses (429) with exponential back-off.
...
@retry(
    ...
    reraise=True,                   # propagate after all retries are exhausted
)
async def fetch_movie_with_credits_async(
    ...
    Returns the parsed and merged JSON dict on success, or None if the request
    fails with a non-retryable status code (4xx except 429).
    """
    try:
        ...
        if status == 429:
            # Rate-limited: re-raise as RequestError so tenacity retries it
            logger.warning(f"Rate limited (429) for movie_id={movie_id}. Retrying...")
            raise httpx.RequestError("Rate limited (429)", request=exc.request) from exc
        if status >= 500:
            logger.error(f"Server error {status} for movie_id={movie_id}. Retrying...")
            raise httpx.RequestError(f"Server error {status}", request=exc.request) from exc

        logger.warning(f"HTTP {status} for movie_id={movie_id} — skipping.")
        return None
...
async def _fetch_all_movies_async(
    movie_ids: list[int], max_concurrent: int = 15
) -> list[dict[str, Any]]:
    ...
    async with _get_async_client() as client:
        ...
        logger.info(
            f"Batch fetch completed: {len(valid_results)} / {len(movie_ids)} "
            f"movies downloaded successfully."
        )
...
def fetch_all_movies(movie_ids: list[int]) -> list[dict[str, Any]]:
    ...
    """
    max_concurrent = get_settings().tmdb.max_concurrent
    return asyncio.run(_fetch_all_movies_async(movie_ids, max_concurrent=max_concurrent))
